...
public class LoggingInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (subtype != null && (subtype.contains("json")
            ...
            if (bodyString.length() > 1024 * 4) {
                //可以支持更大的长度，但是导致低性能盒子极为卡顿，所以还是设置为4KB大小，确保体验流畅度
                Log.w(TAG, "body too long cancel print:" + bodyString.length());
            } else {
                Log.d(TAG, "responseBody.contentLength:" + bodyString.length());
